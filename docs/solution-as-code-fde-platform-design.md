...
`solutionType` 是方案的类型标签，决定平台推荐哪些默认组件、工作流骨架、评测指标和模板字段。`metadata.industry` 是行业标签，只用于行业模板匹配、Prompt 默认值、领域词表和模型参数等配置推荐，不改变能力集合或工作流语义。若两者产生推荐冲突，优先级为：Manifest 显式配置 > `solutionType` 默认骨架 > `metadata.industry` 默认配置。平台内置多种方案模板（客服问答、数据查询、告警升级、审批流程等），FDE 选择模板后只需修改配置和数据源，无需从空白 Manifest 开始。

### 术语与版本约定

- **知识源**：Manifest 中声明的原始或预处理输入文件、表格、规则集。
- **知识单元**：知识流水线产出的可检索、可引用、可校验的最小结构化记录。
- **知识索引**：Runtime 为检索而构建的运行态结构，可删除并由 Manifest 与知识源重建。
- **校验**：`solution validate` 阶段的静态检查。
- **门禁**：评测、知识质量或发布前的阻断性检查。
- **检查**：发布流程中可组合的单项检查，如凭证、入口、可观测性。

`apiVersion` 按兼容性演进。`solution.codex/v1` 内不得破坏已有字段语义；新增字段必须有默认值或被旧 Runtime 忽略。破坏性 Schema 变更必须进入新的 `apiVersion`，并提供迁移说明。组件引用采用语义化版本，MVP 只接受精确版本 pin（如 `@1.0.0`），后续版本再支持版本范围。

关键设计决策：

| 决策 | 依据 |
|------|------|
| Manifest 是唯一可信源 | 避免 PoC、控制台配置和生产环境漂移 |
| MVP 使用线性工作流和单层 `when` | 保持 Validator 可确定实现，避免工作流语言膨胀 |
| 区分 `processor` 与 `action` | 将纯数据处理和副作用隔离，便于审计与重试 |
| Go 作为 Runtime 内核、Python 作为 Worker | Go 适合单二进制交付和在线执行，Python 适合离线文档解析和 AI 工具生态 |
...
- 【Phase 2+】表格型知识（CSV、Excel）：加载为内存表或持久化表，通过扩展后的知识接口提供结构化查询。
- 【Phase 2+】规则型知识：加载为可评估的规则集，通过扩展后的知识接口提供条件匹配。
- 执行基础质量门禁。
- 持久化质量报告。
- 为检索和查询组件提供统一的知识访问接口。
...
**组件实现必须在调用之间是无状态的**。任何需要跨请求共享的数据必须通过 `RuntimeContext` 注入的外部存储接口（如后续版本提供的 `Cache()`、`Store()`）显式管理。平台不保证组件实例的请求隔离——同一 `Component` 实例可能被多个并发请求复用，组件不得在 struct 字段中缓存请求级状态。
...
RuntimeContext 能力调用约束：

- 所有能力调用必须继承当前工作流 run 的 `context.Context`，不得绕过取消、deadline 或 Trace。
- `Model()` 调用默认使用 `runtime.modelPolicy.maxLatencyMs` 作为单次超时；单个工作流 run 的模型调用总耗时不得超过同一上限，MVP 不强制调用次数上限但必须记录 usage 和 cost。
- `Knowledge().Retrieve()` 单次调用默认超时 500ms；超过时返回 `knowledge_timeout`，进入当前节点错误处理。
- `HTTP()` 仅 Phase 2 可用，默认单次超时 3000ms；必须写入 Trace span，记录目标 host、方法、状态码、耗时和脱敏后的错误，不记录密钥、完整 URL query 或敏感 body。
- `Logger()` 输出必须带 `traceId`，并执行与 TraceWriter 一致的脱敏规则。
- `Actions()` 只读，不允许组件修改历史 action 摘要或控制流。
...
| `http-caller` | action | `url` + `method` + `bodyTemplate` → API 调用 | Phase 2 |
...
- `perception.sensors[].config` 在 MVP 中只允许接入类配置：`endpointPath`、`authTokenRef`、`tokenRef`、`schemaVersion`、`adapter`。其中 `adapter` 只能执行客户 payload 到 W2A envelope 的字段映射，不得包含业务过滤、优先级、路由、人工升级或动作执行逻辑；这类业务规则必须进入 workflow 节点。
...
| 字段 | 默认值 | 行为 |
|------|--------|------|
| `workflow.onError.retry` | `0` | 节点失败后不重试，直接按 fallback 或错误返回处理 |
| `workflow.onError.fallbackNode` | 未设置 | hard failure 直接终止工作流并返回错误响应 |
| `workflow.inputMapping` | 未设置 | Runtime 将完整 `RuntimeRequest` 作为工作流级 `inputs`，组件仍只能通过显式节点 `inputs` 或默认传入的 `inputs` 读取 |
| `workflow.nodes[].inputs` | 未设置 | 当前节点收到工作流级 `inputs` 对象，不隐式接收 `step_outputs` |
| `workflow.nodes[].when` | 未设置 | 节点无条件执行 |
| `workflow.nodes[].continueOnFailure` | `false` | 仅 action 节点可设置；processor 节点设置该字段时校验失败 |
| `knowledge.qualityGates` | `[]` | Phase 1 仍执行 JSONL 最小契约检查；Phase 2 完整质量门禁仅执行显式声明项 |
| `runtime.observability.logInputs` | `masked` | 未声明时按脱敏输入写 Trace |
| `runtime.observability.logOutputs` | `true` | 输出写入 Trace，但 PII 与密钥仍必须脱敏 |
| `runtime.observability.retainDays` | 环境默认 `7` | 未声明时使用目标环境运行目录默认保留期 |
| `runtime.modelPolicy.maxLatencyMs` | `8000` | 模型调用默认超时；环境白名单可覆盖 |
| `runtime.modelPolicy.maxCostPerRunUsd` | 不限制 | 未声明时只记录成本，不做阻断 |
| `evaluation.gates[].severity` | `warn` | 未声明 severity 的评测门禁不阻断 release |
| `evaluation.gates[].schedule` | `onRelease` | 未声明 schedule 时只在 release/evaluate 主动执行 |
...
- `RuntimeContext.Env()` 只暴露白名单字段的结构化类型（非 map），组件不得通过 `Env()` 读取非白名单字段。`solution validate` 必须拒绝任何不在白名单中的 `delivery.environments[].config` 字段。白名单是 Manifest API 版本的一部分，随 `apiVersion` 演进。
...
- 幂等缓存只在认证、W2A envelope 校验、Sensor 来源校验和 Signal 类型校验通过后查询或写入；认证失败不参与幂等缓存。若请求命中已声明的 Sensor endpoint，则认证失败只写安全审计日志，不写拒绝类 Trace；若请求无法归属到任何 Solution 或 Sensor（例如未知路径扫描），只写平台安全审计日志。缓存保存成功结果与确定性入口拒绝结果（例如未知版本、必填字段缺失、inputMapping 缺失）；运行时临时失败（例如模型超时、组件异常、网络抖动）不缓存，以便后续重试。重复请求命中缓存时直接返回已缓存结果，并在 Trace 或审计日志中标记为 duplicate。
...
MVP 类型流校验粒度固定为一级扁平字段校验：字段存在性、primitive 类型、array/object 顶层类型。`object` 和 `array` 的内部结构不递归校验；如需被 `when` 或下游节点稳定引用，组件必须把嵌套值展开为一层输出字段。嵌套 Schema、数组元素类型和 JSON Schema 级深度校验留到后续版本。
...
- 如果引用的上游节点因执行错误、hard failure 或未执行而没有成功输出，条件求值视为运行时错误，而不是 false。
- ManifestValidator 必须做保守检查：如果一个节点的 `when` 或 `inputs` 引用了可能被跳过的上游节点输出，则该 Manifest 校验失败。MVP 的安全规则是：被引用节点不能带 `when`，也不能传递依赖带 `when` 的节点；共享同一个前置条件仍视为复杂控制流，MVP 不做等价条件证明，必须改写为无条件上游节点或封装到组件内部。后续版本若引入分支路径标识后，再允许同条件分支内引用。
...
- action 节点输出可以写入 `step_outputs` 并被后续节点通过 `workflow.nodes[].inputs` 显式引用，但这只表示“使用 action 的结构化结果”，不允许下游节点重放、补偿或隐式依赖 action 副作用。action 输出同样受 `component.yaml` 的 `outputSchema` 与一级类型流校验约束。若 action hard fail，除非进入 fallback，否则下游节点不会执行；若 action soft fail 且 `continueOnFailure: true`，其 `{"status":"failed"}` 输出可被后续节点读取。
...
- `knowledge_quality_passed`：按以下顺序查找质量报告 — 1) 当前环境 `tracePath` 下最近一次 `solution run` 或 `solution ingest` 生成的质量报告；2) 如不存在，`solution release` 直接失败，提示 FDE 先执行 `solution run` 或 `solution ingest` 生成质量报告。`solution release` 只检查不修复：报告不存在、fingerprint 不匹配、过期或存在 block 级失败时均直接阻断，不自动生成报告或重建索引。报告必须绑定 `manifest_fingerprint`、`knowledge_config_fingerprint` 和 `knowledge_sources_fingerprint`。其中 `knowledge_config_fingerprint` 覆盖 `knowledge.sources`、`knowledge.schemas`、`knowledge.qualityGates` 和 `runtime.knowledgeBindings`，`knowledge_sources_fingerprint` 由每个知识源内容 hash 与文件元数据共同构成。任一 fingerprint 与当前发布内容不匹配、报告过期（默认 TTL 24 小时）或存在 block 级失败时都必须失败。报告写入采用临时文件 + 原子 rename，避免 release 读取半写入内容。
- `eval_gates_passed`：`solution release` 优先复用满足 fingerprint 匹配且未过期（默认 1 小时）的 onRelease 评测缓存；缓存键必须包含 `solution_name`、`environment_name`、`execution_fingerprint` 和每个 `evaluation.datasets[].uri` 的 `dataset_fingerprint`。`execution_fingerprint` 至少覆盖除非执行性部署字段外的全部 Manifest 内容，并必须包含影响运行结果的环境覆盖字段：`defaultModel`、`fallbackModel`、`maxLatencyMs`、`maxCostPerRunUsd`、模型供应商 endpoint、知识绑定和组件配置。此外必须包含 `knowledge_sources_fingerprint`（由每个知识源内容 hash 与文件元数据共同构成），确保知识源内容变更时评测缓存失效。`tracePath`、`retainDays` 等只影响存储位置或保留期的字段不进入该指纹。`dataset_fingerprint` 由评测数据集内容 hash 与文件元数据共同构成。缓存 miss 时现场执行所有 `schedule: onRelease` 且 `severity: block` 的评测门禁，并将结果回写缓存。`weekly` 门禁只产生告警和报告，不影响 `solution release` 成功退出。
...
- 同时将错误摘要以 `errorSummary` 字段自动注入到 fallback 节点的 `input` 中（`{"failedNode": ..., "message": ..., "type": ..., "attempts": ...}`），组件可直接从 `input.errorSummary` 读取，无需仅通过 `context.error` 隐式获取。自动注入发生在显式 `inputs` 求值之后；如果 Manifest 显式声明 `errorSummary`，校验失败，避免覆盖平台错误上下文。
...
```json
{
  ...
  "version": "string",
  "environment": "string",
  "trigger": {"type": "chat|w2a_signal", "sensor": "string?", "signalType": "string?"},
  ...
  "spans": [
    {
      ...
      "node": "string?",
      "component": "string?",
      "kind": "workflow_node|router|model|knowledge|http|release_check",
      "attempt": 1,
      "input": "object?",
      "output": "object?",
      "latencyMs": 0,
...
- 正常链路：`status: success`，包含每个执行节点的 span，`error: null`。
- 拒绝链路：`status: rejected`，可以没有 workflow node span，但必须包含 router 或入口 span，并写入结构化 `error`。命中已声明 Chat API 或 Sensor endpoint 的入口拒绝（含未知 W2A 版本、未授权 Signal 类型、inputMapping 缺失、请求 Schema 错误）必须生成拒绝类 Trace；认证失败只写安全审计日志，不写拒绝类 Trace；无法归属到 Solution/Sensor 的未知路径或扫描流量只写平台安全审计日志。Trace 不得写入 token 值。
- 异常链路：`status: failed`，保留已完成 span 和失败 span，顶层 `error` 必须与失败 span 的错误摘要一致。
- `input` 与 span `input` 均为脱敏后的归一化输入；`raw_payload` 不写入 Trace。
- `error.message` 不包含 stack、密钥、完整外部响应体或 PII；内部 stack 只能进入本地调试日志，不进入 Trace。
...
错误码采用稳定的 `DOMAIN_REASON` 命名，按入口、Manifest、工作流、组件、知识、评测、发布、安全分域管理。API 响应中的 `code` 必须来自平台错误码枚举；组件只能返回结构化业务状态或平台包装后的组件错误，不能自造对外错误码。
...
| `MANIFEST_VALIDATION_ERROR` | 400 | Manifest 结构、引用、类型流或安全校验失败 |
| `SIGNAL_UNSUPPORTED_VERSION` | 400 | W2A `schema_version` 不受支持 |
| `SIGNAL_UNSUPPORTED_TYPE` | 400 | Signal 类型不在 Sensor 白名单内 |
| `INPUT_MAPPING_ERROR` | 400 | `workflow.inputMapping` 缺失或路径解析失败 |
| `UNAUTHORIZED` | 401 | W2A Signal 认证失败 |
| `NOT_FOUND` | 404 | 端点不存在 |
| `SIGNAL_IDEMPOTENCY_CONFLICT` | 409 | 重复的 `signal_id` |
| `WORKFLOW_FAILED` | 500 | 工作流执行硬失败 |
| `COMPONENT_FAILED` | 500 | 组件返回系统异常或 hard failure |
| `KNOWLEDGE_UNAVAILABLE` | 503 | 知识索引不可用或检索超时 |
...
- `RuntimeContext.Model()` 与 `RuntimeContext.HTTP()` 能力完整实现。
...
- `./deploy/<env>/` 必须是可自包含启动包：包含原始 Manifest 的只读快照、必要的知识源和评测数据集，或在生成说明中声明等价卷挂载；所有知识源、评测集路径应在 Manifest 中统一使用相对于 `$SOLUTION_ROOT` 的路径，容器内通过卷挂载保持相同相对结构。部署包不得重写 Manifest 的业务语义；质量报告的 `manifest_fingerprint` 仍基于源 Manifest 文件内容计算，评测缓存使用源 Manifest 与执行性环境覆盖共同计算的 `execution_fingerprint`。
...
MVP 的非功能目标用于约束实现方案和验收口径，不代表生产 SLA。

| 维度 | MVP 目标或限制 |
|------|----------------|
| 端到端延迟 | Chat 与 W2A 单请求 P95 不超过 `runtime.modelPolicy.maxLatencyMs + 1500ms`；默认模型策略为 8000ms 时，端到端 P95 目标为 9500ms |
| 组件执行超时 | 单个组件继承 workflow run 的 `context.Context` deadline；不得自行创建无 deadline 的后台调用 |
| 模型调用 | 单次模型调用和单个 run 内模型总耗时均受 `runtime.modelPolicy.maxLatencyMs` 约束 |
| 知识检索 | Phase 1 内存关键词检索 P95 目标 500ms；超时返回 `KNOWLEDGE_UNAVAILABLE` 或组件级 `knowledge_timeout` 错误 |
| 并发能力 | 单实例本地运行目标支持 10 个并发 run；超过能力时允许返回 429 或排队，必须写入 Trace 或结构化日志 |
| Trace 写入 | Trace 持久化采用每请求独立文件，写入失败不得吞掉工作流错误；若 Trace 写入失败，响应中必须体现可观测性失败 |
| 知识更新原子性 | `solution ingest` 或启动时知识索引构建必须使用临时目录/临时文件生成完整索引和质量报告，全部成功后再原子替换可见索引 |
| 部署模型 | MVP 以单实例 Runtime 为准；多实例部署需要外部共享幂等存储、共享 Trace/报告存储和一致的知识索引发布机制，不满足时不承诺幂等与报告一致性 |

单实例约束直接影响 W2A 幂等：Phase 1 的进程内 TTL 幂等缓存只在当前 Runtime 进程内有效。Runtime 重启、横向扩容或多副本部署时，必须升级到 Redis/PostgreSQL 等共享幂等存储后才能宣称生产级幂等保护。

知识摄取和索引更新必须满足“全成功或全失败”：加载源文件、生成索引、生成质量报告、写入缓存元数据任一环节失败时，不得替换当前可用索引或当前有效质量报告。Release 只能消费 fingerprint 匹配且原子完成的质量报告。

具体实现约束：索引构建必须使用临时目录（如 `.tmp-index-<pid>`），完成后通过 `os.Rename` 原子替换为最终索引目录。质量报告与索引使用相同的 `build_id`（格式 `{timestamp}-{short_hash}`）关联——release 检查时同时校验质量报告与索引的 `build_id` 一致性，确保两者来自同一次构建。
