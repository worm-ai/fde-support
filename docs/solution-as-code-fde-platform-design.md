...
- 幂等缓存只在认证、W2A envelope 校验、Sensor 来源校验和 Signal 类型校验通过后查询或写入；认证失败不参与幂等缓存。若请求命中已声明的 Sensor endpoint，则认证失败也必须写拒绝类 Trace；若请求无法归属到任何 Solution 或 Sensor（例如未知路径扫描），只写平台安全审计日志。缓存保存成功结果与确定性入口拒绝结果（例如未知版本、必填字段缺失、inputMapping 缺失）；运行时临时失败（例如模型超时、组件异常、网络抖动）不缓存，以便后续重试。重复请求命中缓存时直接返回已缓存结果，并在 Trace 或审计日志中标记为 duplicate。
...
- `knowledge_quality_passed`：按以下顺序查找质量报告 — 1) 当前环境 `tracePath` 下最近一次 `solution run` 或 `solution ingest` 生成的质量报告；2) 如不存在，`solution release` 自行执行一次知识加载和质量门禁检查并生成报告。报告必须绑定 `manifest_fingerprint`、`knowledge_config_fingerprint` 和 `knowledge_sources_fingerprint`。其中 `knowledge_config_fingerprint` 覆盖 `knowledge.sources`、`knowledge.schemas`、`knowledge.qualityGates` 和 `runtime.knowledgeBindings`，`knowledge_sources_fingerprint` 由每个知识源内容 hash 与文件元数据共同构成。任一 fingerprint 与当前发布内容不匹配、报告过期（默认 TTL 24 小时）或存在 block 级失败时都必须失败。报告写入采用临时文件 + 原子 rename，避免 release 读取半写入内容。
- `eval_gates_passed`：`solution release` 优先复用满足 fingerprint 匹配且未过期（默认 1 小时）的 onRelease 评测缓存；缓存键必须包含 `solution_name`、`environment_name`、`execution_fingerprint` 和每个 `evaluation.datasets[].uri` 的 `dataset_fingerprint`。`execution_fingerprint` 至少覆盖除非执行性部署字段外的全部 Manifest 内容，并必须包含影响运行结果的环境覆盖字段：`defaultModel`、`fallbackModel`、`maxLatencyMs`、`maxCostPerRunUsd`、模型供应商 endpoint、知识绑定和组件配置。`tracePath`、`retainDays` 等只影响存储位置或保留期的字段不进入该指纹。`dataset_fingerprint` 由评测数据集内容 hash 与文件元数据共同构成。缓存 miss 时现场执行所有 `schedule: onRelease` 且 `severity: block` 的评测门禁，并将结果回写缓存。`weekly` 门禁只产生告警和报告，不影响 `solution release` 成功退出。
...
- 拒绝链路：`status: rejected`，可以没有 workflow node span，但必须包含 router 或入口 span，并写入结构化 `error`。命中已声明 Chat API 或 Sensor endpoint 的入口拒绝（含认证失败、未知 W2A 版本、未授权 Signal 类型、inputMapping 缺失、请求 Schema 错误）必须生成拒绝类 Trace；无法归属到 Solution/Sensor 的未知路径或扫描流量只写平台安全审计日志。Trace 不得写入 token 值。
